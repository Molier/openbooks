...
func init() {
	...
	desktopCmd.PersistentFlags().StringVar(&globalFlags.AuthUser, "auth-user", "", "Username for HTTP Basic Authentication. If set, --auth-pass must also be set.")
	desktopCmd.PersistentFlags().StringVar(&globalFlags.AuthPass, "auth-pass", "", "Password for HTTP Basic Authentication. If set, --auth-user must also be set.")
	...
	desktopCmd.Flags().BoolVar(&desktopConfig.AutoExtractAll, "auto-extract", false, "Automatically extract all files from downloaded archives and remove the archive.")
}
